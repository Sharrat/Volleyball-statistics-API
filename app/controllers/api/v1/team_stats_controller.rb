  ...
  module V1
    class TeamStatsController < ApplicationController
      def show
        ...
        Match.all.each do |match| # pętla po każdym meczu
        ...
        matches_info = Match.where('team1_id = ? OR team2_id = ?', params[:id].to_i, params[:id].to_i) #porównywanie czy team1_id==params[:id] lub team2_id==params[:id]; params[:id] jest zamieniane na int
        ...
        possition=matches_won_sorted_id.index(params[:id].to_i-1) # Wczytywanie jaka pozycje w tabeli zajmuje druzyna o id params[:id](ale numery zaczynaja się od 0 więc trzeba odjąć 1)
        possition+=1 #zwrócony indeks jest od 0 więc dodajemy 1 (miejsca w tabeli liczone są od jeden - nie ma miejsca zerowego)
